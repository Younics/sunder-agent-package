    ...
    AgentParentRunContinuationService parentRunContinuationService)
{
    ...
    private readonly AgentParentRunContinuationService _parentRunContinuationService = parentRunContinuationService;
    ...
    public async Task<AgentRunCheckpointRecord?> ApproveAsync(Guid sessionId, string requestId)
    {
        ...
        {
            return _sessionService.SaveCheckpoint(sessionId, pending.RunRevision, AgentRunStatus.Failed, "The workspace used for this run was not found.");
        }

        var profile = ResolveProfile(pending.ProfileId);
        if (profile is null)
        {
            return _sessionService.SaveCheckpoint(sessionId, pending.RunRevision, AgentRunStatus.Failed, "The agent profile used for this run was not found.");
        }
        ...
        {
            return _sessionService.SaveCheckpoint(sessionId, pending.RunRevision, AgentRunStatus.Failed, "No installed provider matches this profile yet, or no model is selected.");
        }
        ...
        var runHandle = new AgentActiveRunHandle(pending.RunId, pending.RunRevision, pending.CreatedAtUtc, profile.ProfileId, pending.UserMessage, new CancellationTokenSource());
        ...
        try
        {
            var host = _behaviorLoopHostFactory.Create(provider, session, profile, workspace, pending.RunId, pending.RunRevision, pending.CreatedAtUtc, pending.UserMessage, pending.UserTurnId);
            var approvedToolOutcome = await host.HandleApprovedToolCallAsync(pending, runHandle.CancellationTokenSource.Token).ConfigureAwait(false);
            if (approvedToolOutcome.Kind != AgentToolCallOutcomeKind.Executed)
            {
                _runEventLogger.LogRunEvent(PackageLogLevel.Warning, sessionId, pending.RunId, pending.RunRevision, "permission.approved_resume.failed", approvedToolOutcome.Kind.ToString(), resumeStopwatch.ElapsedMilliseconds);
                return approvedToolOutcome.Checkpoint ?? _sessionService.GetLatestCheckpoint(sessionId);
            }
            ...
            try
            {
                var metadata = await _providerResolver.ResolveRunMetadataAsync(provider, chatBinding, runHandle.CancellationTokenSource.Token).ConfigureAwait(false);
                runCapabilities = metadata.RunCapabilities;
                modelVariant = metadata.ModelVariant;
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                return _sessionService.SaveCheckpoint(sessionId, pending.RunRevision, AgentRunStatus.Failed, ex.Message);
            }

            if (!runCapabilities.SupportsNativeToolCalling)
            {
                return _sessionService.SaveCheckpoint(sessionId, pending.RunRevision, AgentRunStatus.Completed, "Approved tool call executed.");
            }
            ...
            _runEventLogger.LogRunEvent(PackageLogLevel.Information, sessionId, pending.RunId, pending.RunRevision, "permission.approved_resume.completed", loopResult.CompletionKind.ToString(), resumeStopwatch.ElapsedMilliseconds);
            var parentCheckpoint = await _parentRunContinuationService.TryResumeAfterChildCompletionAsync(session, loopResult.Checkpoint, workspace.WorkspaceId, runHandle.CancellationTokenSource.Token).ConfigureAwait(false);
            return parentCheckpoint ?? loopResult.Checkpoint;
        }
        catch (OperationCanceledException)
        {
            _runEventLogger.LogRunEvent(PackageLogLevel.Warning, sessionId, pending.RunId, pending.RunRevision, "permission.approved_resume.canceled", "Approved permission resume was canceled.", resumeStopwatch.ElapsedMilliseconds);
            return _sessionService.GetLatestCheckpoint(sessionId);
        }
        finally
        {
    ...
    public AgentRunCheckpointRecord? Deny(Guid sessionId, string requestId)
    {
        ...
        {
            _parentRunContinuationService.TryRecordParentTaskFailureAfterChildStop(session, stoppedCheckpoint);
        }
