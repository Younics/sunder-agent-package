...
internal static class MemoryRepositorySql
{
    ...
    public static void Write(SqliteConnection connection, SqliteTransaction transaction, StoredMemoryRecord memory, string normalized, bool insert)
    {
        ...
        command.CommandText = insert
            ? """
                INSERT INTO SessionMemories
                    (MemoryId, SessionId, Category, Content, NormalizedContent, EvidenceText, SourceTurnId, Importance, Confidence, IsPinned, State, SupersededByMemoryId, CreatedAtUtc, UpdatedAtUtc, LastAccessedAtUtc, AccessCount, Provenance, MemoryRevision, IsManual)
                VALUES ($memoryId, $sessionId, $category, $content, $normalized, $evidenceText, $sourceTurnId, $importance, $confidence, $isPinned, $state, $supersededByMemoryId, $createdAtUtc, $updatedAtUtc, $lastAccessedAtUtc, $accessCount, $provenance, $memoryRevision, $isManual);
                """
            : """
                UPDATE SessionMemories SET Category = $category, Content = $content, NormalizedContent = $normalized,
                    ...
                    Provenance = $provenance, MemoryRevision = $memoryRevision, IsManual = $isManual
        ...
        if (command.ExecuteNonQuery() != 1)
        {
            throw new InvalidOperationException(
                $"Memory '{memory.MemoryId}' changed or was deleted before its mutation could commit.");
        }
        EmbeddingRepository.DeleteStagingGenerations(connection, transaction, memory.SessionId);
    }
    ...
    public static StoredMemoryRecord Read(SqliteDataReader reader)
        ...
        {
            MemoryRevision = reader.GetInt64(16),
            IsManual = reader.GetInt64(17) != 0,
        };
    ...
    private static void Bind(SqliteCommand command, StoredMemoryRecord memory, string normalized)
    {
        ...
        command.Parameters.AddWithValue("$memoryRevision", memory.MemoryRevision);
        command.Parameters.AddWithValue("$isManual", memory.IsManual ? 1 : 0);
    }
}
