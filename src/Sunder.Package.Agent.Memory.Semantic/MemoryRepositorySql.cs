        ...
        command.CommandText = insert
            ? """
                INSERT INTO SessionMemories
                    (MemoryId, SessionId, Category, Content, NormalizedContent, EvidenceText, SourceTurnId, Importance, Confidence, IsPinned, State, SupersededByMemoryId, CreatedAtUtc, UpdatedAtUtc, LastAccessedAtUtc, AccessCount)
                VALUES ($memoryId, $sessionId, $category, $content, $normalized, $evidenceText, $sourceTurnId, $importance, $confidence, $isPinned, $state, $supersededByMemoryId, $createdAtUtc, $updatedAtUtc, $lastAccessedAtUtc, $accessCount);
                """
            : """
