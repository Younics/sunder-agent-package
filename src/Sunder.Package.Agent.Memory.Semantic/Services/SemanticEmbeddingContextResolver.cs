...
)
{
    ...
    )
    {
        ...
        {
            return SemanticEmbeddingContext.Disabled(
                "Semantic retrieval is disabled in package settings."
            );
        }
        ...
        var binding = string.IsNullOrWhiteSpace(profileId)
            ? GetRuntimeCatalog()
                ?.GetSessionModelBinding(sessionId, AgentModelCapabilityKinds.Embedding)
        ...
        {
            return SemanticEmbeddingContext.Disabled(
                "No embedding provider/model is configured on this agent."
            );
        }

        var provider = _extensionCatalog
            .GetExtensions(PackageExtensionPoints.EmbeddingProviders)
        ...
        if (provider is null)
        {
            return SemanticEmbeddingContext.Unavailable(
                $"Embedding provider '{binding.ProviderId}' is not installed."
            );
        }
        ...
        return SemanticEmbeddingContext.Ready(
            binding.ProviderId,
            binding.ModelId,
