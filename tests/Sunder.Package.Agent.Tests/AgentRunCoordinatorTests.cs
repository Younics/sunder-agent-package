...
public sealed class AgentRunCoordinatorTests
{
    ...
    public async Task QueueUserMessageAsync_BoundedMemoryContext_PreservesHistoricalToolPairs()
    {
        ...
        runtime.SessionService.SaveWorkingSummary(
            sessionId,
            "Existing memory summary forces bounded historical context."
        );
    ...
    public async Task QueueUserMessageAsync_KeepsEntireActiveExchangeWhenWorkingSummaryBoundsOlderHistory()
    {
        ...
        runtime.SessionService.SaveWorkingSummary(
            sessionId,
            "Existing working summary for earlier turns."
        );
    ...
    public async Task DenyPendingPermission_AppendsErrorToolResultAndAllowsNextMessage()
    {
        ...
        var deniedCheckpoint = runtime.RunCoordinator.DenyPendingPermission(
            sessionId,
            pending.RequestId
        );
    ...
    public void AgentSessionsViewModel_CompactLayout_UsesRowActivationAndReturnsToList()
    {
        ...
        viewModel.ActivateSession(
            viewModel.Sessions.Single(item => item.SessionId == session.SessionId)
        );
    ...
    [Fact]
    public void AgentSessionsViewModel_NormalListUpdates_LeaveFooterStatusEmpty()
    {
        using var runtime = AgentTestRuntime.Create(
            new ScriptedProvider((_, _) => Complete("done"))
        );
        ...
        runtime.SessionService.CreateSession("Beta Session");
        ...
        runtime.SessionService.SaveCheckpoint(
            session.SessionId,
            runtime.SessionService.GetNextRunRevision(session.SessionId),
            AgentRunStatus.Running,
    ...
    public async Task BuildInstructionContextAsync_PassesRecentLiveBufferAndFormatsExplainableRecall()
    {
        ...
        Assert.Contains(
            "[preference | Active] Prefer concise answers.",
    ...
    public async Task MemorySemanticFeature_HandleLifecycleEventAsync_PromotesToolResultProjectFacts()
    {
        ...
        Assert.Contains(
            promoted,
            memory =>
                memory.Category == "project-fact"
                && memory.Content.Contains("TargetFramework: net10.0", StringComparison.Ordinal)
        );
    }
    ...
    public void MemoryInspectorService_CreateCorrectedMemory_LinksSupersession()
    {
        ...
        var retrievalBackend = new SemanticMemoryRetrievalBackend(
            store,
            new ProfileConfiguredEmbeddingProviderResolver(extensionCatalog),
            settings
        );
    ...
    public async Task MemoryInspectorService_GetSemanticIndexStatus_ReportsIndexedState()
    {
        ...
        var retrievalBackend = new SemanticMemoryRetrievalBackend(
            store,
            new ProfileConfiguredEmbeddingProviderResolver(runtime.ExtensionCatalog),
            settings
        );
        var indexingBackgroundService = new SemanticMemoryIndexingBackgroundService(
            store,
            ...
            IAgentLifecycleObserver
    {
        ...
        public ValueTask<AgentLifecycleObserverResult?> HandleLifecycleEventAsync(
